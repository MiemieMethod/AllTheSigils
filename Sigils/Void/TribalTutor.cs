...
namespace AllTheSigils
{
    public partial class Plugin
    {
        ...
        private void AddTribalTutor()
        {
            ...
            Texture2D tex_a1 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor);
            ...
            void_TribalTutor.ability = SigilUtils.CreateAbilityWithDefaultSettingsKCM(rulebookName, rulebookDescription, typeof(void_TribalTutor), tex_a1, tex_a2, LearnDialogue,
                                                                                    true, powerlevel, LeshyUsable, part1Shops, canStack).ability;
            if (Plugin.GenerateWiki)
            {
                Plugin.SigilWikiInfos[void_TribalTutor.ability] = new Tuple<string, string>("void_TribeTutor", "");
            }
        }
    }
    ...
    public class TribalTutorPatch
    {
        ...
        public static void Postfix(ref Texture __result, ref CardInfo info, ref AbilityInfo ability)
        {
            if (ability.ability == void_TribalTutor.ability)
            {
                if (info != null)
                {
                    Texture2D tex1 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor_bird);

                    Texture2D tex2 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor_canine);

                    Texture2D tex3 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor_hooved);

                    Texture2D tex4 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor_insect);

                    Texture2D tex5 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor_reptile);

                    Texture2D tex6 = SigilUtils.LoadTextureFromResource(Art.void_TribeTutor_none);
